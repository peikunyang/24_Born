...
- ion charges are set to zero after the CHARMM system is created
- ion vdW/LJ parameters are unchanged from the CHARMM force field
...
def set_ion_charges_to_zero(nbforce: NonbondedForce, ion_indices: Sequence[int]) -> None:
    """Set only the electrostatic charges of the selected ions to zero.

    The sigma and epsilon values stored in NonbondedForce are preserved.
    The CHARMM LJ/vdW terms in CustomNonbondedForce are not touched.
    """
    for idx in ion_indices:
        charge, sigma, epsilon = nbforce.getParticleParameters(idx)
        nbforce.setParticleParameters(idx, 0.0 * elementary_charge, sigma, epsilon)
...
def write_run_settings(out_dir: Path, args, ff_files: Sequence[str]):
    ...
    with path.open("w", encoding="utf-8") as f:
        ...
        f.write("ion_charges = set to zero after ForceField.createSystem\n")
        f.write("ion_vdw_LJ_parameters = unchanged from CHARMM force field\n")
...
def run_one_distance(args, ff, ff_files: Sequence[str], d: int, platform, props) -> Dict[str, object]:
    ...
    # Zero charge reference run.
    # Only the ion electrostatic charges are changed.
    # Ion LJ/vdW parameters and all water parameters remain unchanged.
    set_ion_charges_to_zero(nbforce, ion_indices)
    current_charges = [charge_e_from_nonbonded(nbforce, idx) for idx in ion_indices]
    ...
    md_settings = {
        ...
        "ion_charges_set_to_zero": "True",
        "ion_vdw_LJ_parameters_unchanged": "True",
        ...
        "MD_ion_charges_e": " ".join(f"{x:.10f}" for x in current_charges),
        "nonbondedMethod": "NoCutoff",
        "requested_d_A": d,
    ...
    print(f"  original ion charges from FF: {original_charges[0]:.3f}, {original_charges[1]:.3f} e")
    print(f"  MD ion charges after zeroing: {current_charges[0]:.3f}, {current_charges[1]:.3f} e")
    ...
    return {
        ...
        "ion1_charge_e": current_charges[0],
        "ion2_charge_e": current_charges[1],
        "ion1_original_charge_e": original_charges[0],
        "ion2_original_charge_e": original_charges[1],
        "waters": len(waters),
...
def main():
    ...
    print("This script sets both ion charges to zero. It keeps ion vdW/LJ parameters unchanged and fixes ion positions by setting ion masses to zero.")
    ...
    print("  ion charges: set to zero")
    print("  ion vdW/LJ parameters: unchanged")
