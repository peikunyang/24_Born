#!/usr/bin/env python3
"""
Sequential single-ion charge-scaling MD for CHARMM TIP3P water clusters in OpenMM.

Use cases
---------
K  : single_K_sel21A_1118w.pdb,  q =  0.0 to  1.0 step  0.1
Cl : single_Cl_sel21A_1118w.pdb, q =  0.0 to -1.0 step -0.1
Mg : single_Mg_sel21A_1118w.pdb, q =  0.0 to  2.0 step  0.2
...
- rigidWater = True
- timestep = 2 fs
- ion mass is set to zero: fixed at origin, but interactions retained
- ion vdW/LJ parameters are unchanged
- only ion charge is changed through NonbondedForce
- spherical wall on water oxygen atoms, R = 20 A
- sequential charging: q0 -> q1 -> ... uses previous final positions/velocities
- each charge state: 400 ps equilibration + 10 ns production
- DCD saved every 100 fs

Outputs
-------
out_dir/
  forcefield_parameters.txt       Human-readable parameter report
  run_settings.txt                MD settings
  summary.csv                     Per-charge summary
  q0.0/system.xml                 Exact OpenMM System XML for that charge state
  q0.0/equil_state.csv
  q0.0/traj.dcd
  q0.0/state.csv
  q0.0/final.pdb
  q0.0/checkpoint.chk
...
energy acoef(type1,type2)/r^12 - bcoef(type1,type2)/r^6. Therefore, the report
extracts charges from NonbondedForce and LJ pair parameters from CustomNonbondedForce.
"""
...
def q_label(q: float) -> str:
    """Folder-safe charge label."""
    q_rounded = round(q, 10)
    if abs(q_rounded) < 5e-11:
        return "q0.0"
    if q_rounded < 0:
        return f"qm{abs(q_rounded):.1f}"
    return f"q{q_rounded:.1f}"


def make_charge_list(q_start: float, q_end: float, q_step: float) -> List[float]:
    if q_step == 0:
        raise ValueError("q_step must not be zero")
    if q_end > q_start and q_step < 0:
        raise ValueError("q_step sign is inconsistent with q_start/q_end")
    if q_end < q_start and q_step > 0:
        raise ValueError("q_step sign is inconsistent with q_start/q_end")

    charges = []
    q = q_start
    tol = abs(q_step) * 1e-6 + 1e-12
    if q_step > 0:
        while q <= q_end + tol:
            charges.append(round(q, 10))
            q += q_step
    else:
        while q >= q_end - tol:
            charges.append(round(q, 10))
...
def find_single_ion_atom(topology, ion_label: str) -> int:
    allowed_resnames = ION_RESNAMES[ion_label]
    expected_element = ION_ELEMENTS[ion_label]
    candidates = []
    for atom in topology.atoms():
        if atom.residue.name in allowed_resnames or atom.element == expected_element:
            candidates.append(atom)
    if len(candidates) != 1:
        details = [(a.index, a.name, a.residue.name, str(a.element)) for a in candidates]
        raise RuntimeError(f"Expected exactly one {ion_label} ion atom, found {len(candidates)}: {details}")
...
def find_water_atoms(topology) -> Tuple[List[int], List[Tuple[int, int, int, object]]]:
    """Return water oxygen indices and first-water triplets (O,H1,H2,residue)."""
    oxygens = []
    waters = []
...
def recenter_positions_on_atom(positions, atom_index: int):
    center = positions[atom_index].value_in_unit(angstrom)
    centered = []
    for p in positions:
...
def get_lj_force(system) -> Optional[CustomNonbondedForce]:
    ...
    if not candidates:
        return None
    if len(candidates) > 1:
        # Prefer explicitly named LennardJones force.
        for f in candidates:
...
def lj_pair_from_acoef_bcoef(force: CustomNonbondedForce, type_i: int, type_j: int) -> Dict[str, float]:
...
def set_ion_charge(nbforce: NonbondedForce, atom_index: int, charge_e: float):
    old_charge, sigma, epsilon = nbforce.getParticleParameters(atom_index)
    nbforce.setParticleParameters(atom_index, charge_e * elementary_charge, sigma, epsilon)
...
def write_forcefield_parameter_report(
    ...
    ion_label: str,
    ion_index: int,
    original_ion_mass_da: float,
    scan_charges: Sequence[float],
    ...
    ion_atom = topology_atom_by_index(topology, ion_index)
    ...
    if ljforce is not None:
        ...
        pairs = [
            ("ion-ion", "ion", "ion"),
            ("ion-water_O", "ion", "water_O"),
            ("ion-water_H", "ion", "water_H1"),
    ...
    with out_path.open("w", encoding="utf-8") as f:
        ...
        f.write("Ion\n")
        f.write("---\n")
        f.write(f"ion_label = {ion_label}\n")
        f.write(f"atom_index = {ion_index}\n")
        f.write(f"residue_name = {ion_atom.residue.name}\n")
        f.write(f"atom_name = {ion_atom.name}\n")
        f.write(f"element = {ion_atom.element}\n")
        f.write(f"original_mass_Da = {original_ion_mass_da:.10f}\n")
        f.write(f"MD_mass_Da = {mass_da(system, ion_index):.10f}\n")
        f.write(f"original_charge_from_NonbondedForce_e = {charge_e_from_nonbonded(nbforce, ion_index):.10f}\n")
        f.write("scan_charges_e = " + " ".join(f"{q:.1f}" for q in scan_charges) + "\n")
        if ljforce is not None:
            f.write(f"LJ_particle_type_index = {lj_type['ion']}\n")
            write_lj_pair_section(f, "ion-ion LJ from CustomNonbondedForce", pair_lj["ion-ion"])
        else:
        ...
        for label, idx, atom_obj in [("O", first_o, o_atom), ("H1", first_h1, h1_atom), ("H2", first_h2, h2_atom)]:
            f.write(f"  {label}: atom={atom_obj.name}, index={idx}, mass_Da={mass_da(system, idx):.10f}, charge_e={charge_e_from_nonbonded(nbforce, idx):.10f}\n")
        f.write("\n")

        if ljforce is not None:
            f.write("Water/ion LJ particle type indices from CustomNonbondedForce:\n")
            for key in ["ion", "water_O", "water_H1", "water_H2"]:
                f.write(f"  {key} = {lj_type[key]}\n")
            f.write("\n")
            for label in ["ion-water_O", "ion-water_H", "water_O-water_O", "water_O-water_H", "water_H-water_H"]:
                write_lj_pair_section(f, f"{label} LJ from CustomNonbondedForce", pair_lj[label])
        f.write("\n")

        f.write("Water intramolecular geometry from initial centered PDB:\n")
...
def write_run_settings(out_dir: Path, args, scan_charges: Sequence[float], ff_files: Sequence[str], ion_index: int, n_waters: int, n_wall: int):
    path = out_dir / "run_settings.txt"
    with path.open("w", encoding="utf-8") as f:
        f.write("Run settings\n")
        f.write("============\n\n")
        f.write(f"input_pdb = {args.pdb}\n")
        f.write(f"ion = {args.ion}\n")
        f.write(f"ion_atom_index = {ion_index}\n")
        f.write("force_field_files = " + " ".join(ff_files) + "\n")
        f.write("scan_charges_e = " + " ".join(f"{q:.1f}" for q in scan_charges) + "\n")
        f.write(f"waters_detected = {n_waters}\n")
        f.write(f"water_oxygens_with_wall = {n_wall}\n")
...
def write_q_info(q_dir: Path, args, q: float, q_index: int, equil_steps: int, prod_steps: int, dcd_interval: int, state_interval: int):
    path = q_dir / "q_info.txt"
    with path.open("w", encoding="utf-8") as f:
        f.write("Charge-state information\n")
        f.write("========================\n\n")
        f.write(f"ion = {args.ion}\n")
        f.write(f"charge_state_index = {q_index}\n")
        f.write(f"charge_e = {q:.10f}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Sequential single-ion charge-scaling MD in CHARMM TIP3P water cluster.")
    parser.add_argument("--ion", required=True, choices=["K", "Cl", "Mg"], help="Ion type: K, Cl, or Mg")
    parser.add_argument("--pdb", required=True, help="Input single-ion water-cluster PDB")
    parser.add_argument("--out_dir", required=True, help="Output directory")
    parser.add_argument("--ff", nargs="+", default=["charmm36.xml", "charmm36/water.xml"], help="OpenMM CHARMM XML files")
    parser.add_argument("--q_start", type=float, default=None, help="Override charge start")
    parser.add_argument("--q_end", type=float, default=None, help="Override charge end")
    parser.add_argument("--q_step", type=float, default=None, help="Override charge step")
    ...
    parser.add_argument("--no_system_xml", action="store_true", help="Do not write per-charge system.xml files")
    args = parser.parse_args()

    if args.test_short:
        args.equil_ps = 1.0
        args.prod_ns = 0.002  # 2 ps
        args.traj_interval_fs = 100.0
        args.state_interval_ps = 0.2
        print("TEST MODE: equil_ps=1, prod_ps=2, traj_interval_fs=100")

    q0, q1, dq = DEFAULT_CHARGE_SCHEDULES[args.ion]
    if args.q_start is not None:
        q0 = args.q_start
    if args.q_end is not None:
        q1 = args.q_end
    if args.q_step is not None:
        dq = args.q_step
    scan_charges = make_charge_list(q0, q1, dq)
    ...
    print("Input PDB:", args.pdb)

    ion_index = find_single_ion_atom(pdb.topology, args.ion)
    water_oxygen_indices, waters = find_water_atoms(pdb.topology)
    print("Ion atom index:", ion_index)
    print("Waters detected:", len(waters))
    print("Water oxygens restrained by spherical wall:", len(water_oxygen_indices))

    centered_positions = recenter_positions_on_atom(pdb.positions, ion_index)
    ...
    nbforce = get_nonbonded_force(system)
    original_ion_mass = mass_da(system, ion_index)
    original_ion_charge = charge_e_from_nonbonded(nbforce, ion_index)

    n_wall = add_spherical_wall(system, water_oxygen_indices, args.wall_radius_a, args.wall_k_kcal_mol_A2)
    print(f"Added spherical wall to {n_wall} water oxygen atoms.")

    # Fix ion at the origin while preserving all interactions.
    system.setParticleMass(ion_index, 0.0 * dalton)
    ...
    md_settings = {
        ...
        "constraints": "HBonds",
        "rigidWater": "True",
        "ion_mass_set_to_zero": "True",
        "original_ion_mass_Da": f"{original_ion_mass:.10f}",
        "original_ion_charge_e": f"{original_ion_charge:.10f}",
        "nonbondedMethod": "NoCutoff",
    }
    ...
    write_forcefield_parameter_report(
        ...
        ion_label=args.ion,
        ion_index=ion_index,
        original_ion_mass_da=original_ion_mass,
        scan_charges=scan_charges,
        ff_files=ff_files,
        md_settings=md_settings,
    )
    write_run_settings(out_dir, args, scan_charges, ff_files, ion_index, len(waters), n_wall)
    ...
    print("OpenMM platform:", platform_name)
    ...
    print(f"  equilibration: {args.equil_ps} ps = {equil_steps} steps per charge")
    print(f"  production: {args.prod_ns} ns = {prod_steps} steps per charge")
    print(f"  DCD interval: {args.traj_interval_fs} fs = every {dcd_interval} steps")
    print(f"  state interval: {args.state_interval_ps} ps = every {state_interval} steps")
    ...
    print("  ion mass set to zero: ion is fixed, interactions are retained")

    records = []

    for idx, q in enumerate(scan_charges):
        label = q_label(q)
        q_dir = out_dir / label
        q_dir.mkdir(parents=True, exist_ok=True)
        print(f"\n===== Charge state {label}: {args.ion} charge = {q:.1f} e =====")

        set_ion_charge(nbforce, ion_index, q)
        nbforce.updateParametersInContext(simulation.context)

        if not args.no_system_xml:
            with (q_dir / "system.xml").open("w", encoding="utf-8") as f:
                f.write(XmlSerializer.serialize(system))

        write_q_info(q_dir, args, q, idx, equil_steps, prod_steps, dcd_interval, state_interval)
        ...
        # Equilibration without DCD trajectory.
        simulation.reporters = []
        simulation.reporters.append(
            StateDataReporter(
                str(q_dir / "equil_state.csv"),
                state_interval,
                step=True,
                time=True,
        ...
        # Production with DCD trajectory.
        simulation.reporters = []
        simulation.reporters.append(DCDReporter(str(q_dir / "traj.dcd"), dcd_interval))
        simulation.reporters.append(
            StateDataReporter(
                str(q_dir / "state.csv"),
                state_interval,
        ...
        with (q_dir / "final.pdb").open("w", encoding="utf-8") as f:
            PDBFile.writeFile(pdb.topology, final_positions, f, keepIds=True)
        simulation.saveCheckpoint(str(q_dir / "checkpoint.chk"))
    ...
    print(f"Parameter report: {out_dir / 'forcefield_parameters.txt'}")
